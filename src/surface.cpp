...
Surface::Surface(std::shared_ptr<Renderer> renderer,
                 std::unique_ptr<Pixmap> pixmap)
    : device_(), renderer_(std::move(renderer)), pixmap_(std::move(pixmap)) {
...
std::unique_ptr<Surface> Surface::MakeRaster(i32 w, i32 h, PixelFormat fmt) {
  ...
  return std::unique_ptr<Surface>(
      new Surface(std::move(renderer), std::move(pixmap)));
}

std::unique_ptr<Surface> Surface::MakeRasterDirect(const PixmapInfo &info,
                                                   void *pixels) {
...
Surface::MakeGpu(const std::shared_ptr<GpuContext> &context, i32 w, i32 h,
                 PixelFormat fmt) {
...
void Surface::endFrame() { device_.endFrame(); }

void Surface::flush() {
  auto recording = device_.finishRecording();
  if (!recording || !renderer_)
    return;
...
bool Surface::isGPU() const { return renderer_ && !pixmap_; }

Pixmap *Surface::peekPixels() { return pixmap_.get(); }

const Pixmap *Surface::peekPixels() const { return pixmap_.get(); }

PixelData Surface::getPixelData() const {
  if (pixmap_) {
    const auto &info = pixmap_->info();
    return PixelData(pixmap_->addr(), info.width, info.height, info.stride,
                     info.format);
  }
  return PixelData();
}
