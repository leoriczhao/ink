...
bool GpuContext::valid() const { return impl_ && impl_->valid(); }

void GpuContext::beginFrame(Color clearColor) {
  if (impl_)
    impl_->beginFrame(clearColor);
}
...
void GpuContext::resize(i32 w, i32 h) {
  if (impl_)
    impl_->resize(w, h);
}
