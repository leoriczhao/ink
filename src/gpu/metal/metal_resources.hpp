...
inline simd_float4x4 metalOrthoProjection(float w, float h) {
  return simd_float4x4{{{2.0f / w, 0, 0, 0},
                        {0, -2.0f / h, 0, 0},
                        {0, 0, -1, 0},
                        {-1.0f, 1.0f, 0, 1}}};
}
...
public:
  ...
  bool init(id<MTLDevice> device, id<MTLLibrary> library, NSString *vertFunc,
            NSString *fragFunc, MTLPixelFormat pixelFormat,
...
public:
  ...
  MTLRenderPassDescriptor *renderPassDescriptor(Color clearColor = {
                                                    0, 0, 0, 255}) const {
    ...
    desc.colorAttachments[0].clearColor =
        MTLClearColorMake(clearColor.r / 255.0, clearColor.g / 255.0,
