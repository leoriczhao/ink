...
Image::Image(const PixmapInfo &info, const void *pixels,
             std::unique_ptr<Pixmap> owned)
    : id_(nextImageId()), storageType_(StorageType::CpuPixmap), info_(info),
      pixels_(pixels), ownedPixmap_(std::move(owned)) {}

Image::Image(const PixmapInfo &info, u64 textureHandle,
             std::shared_ptr<void> lifetimeToken)
    : id_(nextImageId()), storageType_(StorageType::GpuTexture), info_(info),
...
Image::MakeFromBackendTexture(u64 textureHandle, i32 width, i32 height,
                              PixelFormat fmt,
                              std::shared_ptr<void> lifetimeToken) {
  ...
  return std::shared_ptr<Image>(
      new Image(info, textureHandle, std::move(lifetimeToken)));
}

std::shared_ptr<Image>
Image::MakeFromGLTexture(u32 textureId, i32 width, i32 height, PixelFormat fmt,
                         std::shared_ptr<void> lifetimeToken) {
