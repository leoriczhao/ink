 ...
 *
 * On macOS with Metal:
 *   cmake -B build -DINK_BUILD_EXAMPLES=ON -DINK_ENABLE_METAL=ON && cmake
 * --build build
 *
 * On Linux with OpenGL:
 *   cmake -B build -DINK_BUILD_EXAMPLES=ON -DINK_ENABLE_GL=ON && cmake --build
 * build
 *
 *   ./build/example_gpu
 */
...
static void drawScene(ink::Canvas *canvas, int W, int H, float t) {
  ...
  for (int i = 0; i < 6; ++i) {
    ...
    canvas->fillRect({x - size / 2, y - size / 2, size, size},
                     {ink::u8(100 + i * 25), ink::u8(150 - i * 20),
                      ink::u8(200 - i * 15), alpha});
  }
...
#else
#error                                                                         \
    "No GPU backend available. Build with -DINK_ENABLE_METAL=ON or -DINK_ENABLE_GL=ON"
#endif
...
int main(int argc, char *argv[]) {
  ...
  char title[128];
  std::snprintf(title, sizeof(title), "ink - GPU Rendering (%s)",
                backendName());
  ...
  SDL_Window *window = SDL_CreateWindow(
      title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, W, H, windowFlags);
  if (!window) {
