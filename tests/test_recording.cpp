...
TEST(Recording, SetClipPathRecorded) {
  Recorder rec;
  Path p;
  ...
  auto recording = rec.finish();
  ASSERT_EQ(recording->ops().size(), 1u);
  EXPECT_EQ(recording->ops()[0].type, DrawOp::Type::SetClipPath);
  ASSERT_EQ(recording->paths().size(), 1u);
  EXPECT_FALSE(recording->paths()[0].isEmpty());

  MockVisitor visitor;
  recording->accept(visitor);
  ASSERT_EQ(visitor.calls.size(), 1u);
  EXPECT_EQ(visitor.calls[0].kind, VisitorCall::Kind::SetClipPath);
}
