...
// This allows creating images with arbitrary alpha (including fully
// transparent) without going through the rendering pipeline which clears to
// opaque black.
...
TEST(Composite, ImageAtOffsetOnlyAffectsRegion) {
  ...
  for (i32 y = 0; y < DST_H; ++y) {
    for (i32 x = 0; x < DST_W; ++x) {
      bool inImage =
          (x >= OFF_X && x < OFF_X + IMG_W && y >= OFF_Y && y < OFF_Y + IMG_H);
