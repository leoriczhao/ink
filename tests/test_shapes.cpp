...
TEST(Shapes, FillRoundRectCornersDifferFromRect) {
  // Fill a regular rect and a rounded rect of the same size, compare corner
  // pixel The rounded rect should NOT fill the extreme corner pixel.
  ...
  // Top-left corner of the rect at (10,10) - the regular rect should have it
  // colored
  u32 rectCorner = px1[10 * 120 + 10];
  u8 rectR = rectCorner & 0xFF;
  EXPECT_EQ(rectR, 255);

  // The rounded rect should NOT have the extreme corner colored (it's in the
  // rounded-off area)
...
struct ShapeMockVisitor : public DrawOpVisitor {
  ...
  void visitSetClip(Rect) override { types.push_back(DrawOp::Type::SetClip); }
  void visitSetClipPath(const Path &, u8, bool) override {
    types.push_back(DrawOp::Type::SetClipPath);
  }
  void visitClearClip() override { types.push_back(DrawOp::Type::ClearClip); }
  ...
  void visitStrokeRoundRect(Rect, f32, f32, Color, f32, BlendMode,
                            u8) override {
  ...
  void visitFillPath(const Path &, Color, BlendMode, u8, u8, bool,
    ...
    types.push_back(DrawOp::Type::FillPath);
  }
  void visitStrokePath(const Path &, Color, f32, BlendMode, u8, u8, u8, f32,
                       bool, const Shader *, const ColorFilter *) override {
    types.push_back(DrawOp::Type::StrokePath);
  }
  void visitDrawImageRect(const Image *, Rect, Rect, BlendMode, u8,
                          FilterQuality) override {
    types.push_back(DrawOp::Type::DrawImageRect);
  }
};
