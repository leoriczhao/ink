...
template <typename Fn> static std::unique_ptr<Recording> record(Fn fn) {
  Recorder rec;
  fn(rec);
  return rec.finish();
}
...
TEST(DrawPass, SingleOp) {
  auto recording =
      record([](Recorder &r) { r.fillRect({0, 0, 10, 10}, {255, 0, 0, 255}); });
  auto pass = DrawPass::create(*recording);
...
TEST(DrawPass, SameTypeGroupedTogether) {
  ...
  for (size_t i = 0; i < idx.size(); ++i) {
    if (idx[i] == 0)
      posOfFill0 = static_cast<int>(i);
    if (idx[i] == 1)
      posOfStroke = static_cast<int>(i);
    if (idx[i] == 2)
      posOfFill2 = static_cast<int>(i);
  }
...
TEST(DrawPass, ClearClipAtEndOfGroup) {
  ...
  for (size_t i = 0; i < idx.size(); ++i) {
    if (idx[i] == 0)
      posSetClip = static_cast<int>(i);
    if (idx[i] == 1)
      posFill = static_cast<int>(i);
    if (idx[i] == 2)
      posStroke = static_cast<int>(i);
    if (idx[i] == 3)
      posClearClip = static_cast<int>(i);
  }
...
TEST(DrawPass, DifferentTypesSortedByTypeWithinGroup) {
  ...
  auto recording = record([](Recorder &r) {
    ...
    r.strokeRect({0, 0, 5, 5}, {0, 0, 0, 255},
                 1.0f); // index 2, type StrokeRect (1)
  });
