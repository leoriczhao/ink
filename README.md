...
| `INK_VERSION` | auto | Override generated version metadata (`X.Y.Z`) |
...
Install first, then add the install include path and link against the installed
library. `ink/version.hpp` is generated by CMake, so copying the source
`include/` tree directly is incomplete.

```bash
cmake -B build -DINK_VERSION=1.2.3
cmake --build build
...
│   ├── version.hpp.in       # Template for generated version info
...
# Or manually after CMake has generated build/Doxyfile and version.hpp
doxygen build/Doxyfile                    # API reference -> docs/_build/doxygen/html/
INK_VERSION=$(awk -F '"' '/PROJECT_NUMBER/ { print $2 }' build/Doxyfile) \
