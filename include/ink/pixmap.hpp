...
/**
 * @file pixmap.hpp
 * @brief Pixel format, pixel buffer descriptor, and owning/non-owning pixel
 * buffer.
 */
...
struct PixmapInfo {
  ...
  PixelFormat format = PixelFormat::BGRA8888; ///< Pixel format (BGRA is native
                                              ///< on most platforms).
  ...
  static PixmapInfo MakeRGBA(i32 w, i32 h) {
    return Make(w, h, PixelFormat::RGBA8888);
  }
  ...
  static PixmapInfo MakeBGRA(i32 w, i32 h) {
    return Make(w, h, PixelFormat::BGRA8888);
  }
};

/// @brief Owning or non-owning pixel buffer.
///
/// Use Alloc() to create an owned buffer, or Wrap() to reference external
/// memory.
class Pixmap {
public:
  ...
  bool valid() const {
    return pixels_ != nullptr && info_.width > 0 && info_.height > 0;
  }
