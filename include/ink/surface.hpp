...
public:
  ...
  static std::unique_ptr<Surface>
  MakeRaster(i32 w, i32 h, PixelFormat fmt = PixelFormat::BGRA8888);

  /// @brief Create a CPU raster surface wrapping a caller-owned pixel buffer
  /// (zero-copy).
  /// @param info Pixmap descriptor for the external buffer.
  /// @param pixels Pointer to the caller-owned pixel data.
  /// @return Unique pointer to the new Surface.
  static std::unique_ptr<Surface> MakeRasterDirect(const PixmapInfo &info,
                                                   void *pixels);

  /// @brief Create a GPU surface. Falls back to CPU if context is null or
  /// invalid.
  ...
  /// @return Unique pointer to the new Surface.
  static std::unique_ptr<Surface>
  MakeGpu(const std::shared_ptr<GpuContext> &context, i32 w, i32 h,
          PixelFormat fmt = PixelFormat::BGRA8888);

  /// @brief Create a recording-only surface (no rendering, just captures
  /// commands).
  /// @param w Width in pixels.
  /// @param h Height in pixels.
  /// @return Unique pointer to the new Surface.
  ...
  /// @param clearColor The color to clear the render target to (default: opaque
  /// black).
  ...
  /// @return Pointer to the underlying Pixmap, or nullptr for GPU/recording
  /// surfaces.
  Pixmap *peekPixels();
