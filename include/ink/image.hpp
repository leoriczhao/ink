...
public:
  ...
  /// @brief Create an image wrapping existing pixel data (caller must keep data
  /// alive).
  ...
  /// @param textureHandle Opaque handle (e.g. GLuint for GL, VkImage for
  /// Vulkan).
  /// @param width Texture width in pixels.
  /// @param height Texture height in pixels.
  /// @param fmt Pixel format (default RGBA8888).
  /// @param lifetimeToken Optional shared_ptr whose destructor releases the
  /// texture.
  /// @return Shared pointer to the new Image.
  static std::shared_ptr<Image>
  MakeFromBackendTexture(u64 textureHandle, i32 width, i32 height,
  ...
  /// @return Shared pointer to the new Image.
  static std::shared_ptr<Image>
  MakeFromGLTexture(u32 textureId, i32 width, i32 height,
                    PixelFormat fmt = PixelFormat::RGBA8888,
                    std::shared_ptr<void> lifetimeToken = nullptr);
  ...
  bool valid() const {
    if (info_.width <= 0 || info_.height <= 0)
      return false;
    if (storageType_ == StorageType::CpuPixmap)
...
private:
  ...
  Image(const PixmapInfo &info, const void *pixels,
        std::unique_ptr<Pixmap> owned);
  Image(const PixmapInfo &info, u64 textureHandle,
